# Aluminium projections using Machine Learning
...
# Import historical alum data and gdp
alum_df = pd.read_csv('./data/production_and_trade/production_aluminium/alum_usgs_cleaned.csv')
# Remove unneeded column
alum_df = alum_df.iloc[:, [0, 2, 3, 4, 5]] 
...
economy_list = list(alum_df['economy_code'].unique())
# Remove Brunei
economy_list = [i for i in economy_list if i != '10_MAS']
...
for economy in economy_list:
    ...
    # Target variable: aluminium production
    target_df = alum_df[alum_df['economy_code'] == economy].copy().dropna().reset_index(drop = True)
    ...
    full_df = full_df[['year', 'value_x', 'value_y']].rename(columns = {'value_x': 'alum',
                                                                        'value_y': 'real_GDP'}).copy()
    
    full_df = pd.merge(left = full_df, right = pop, on = 'year', how = 'outer')\
        .copy().rename(columns = {'value': 'population'})

    full_df = full_df[['year', 'alum', 'real_GDP', 'population']].copy()

    # Full data frame with aluminium (target only for historical) and real_GDP, population, and GDP per capita features to 2100
    full_df = pd.merge(left = full_df, right = gdp_pc, on = 'year', how = 'outer')\
        .copy().rename(columns = {'value': 'GDP_per_capita'})\
            [['alum', 'year', 'real_GDP', 'population', 'GDP_per_capita']]
    ...
    # STEP 1: natural log transform independent variables and target variable (alum)

    hist_df['alum'] = np.log(hist_df['alum'])
    ...
    for year in hist_df.index:    
        for i in range(1, LAGS + 1):
            if (year - i > 1979) & (year - i in hist_df.index):
                hist_df.loc[year, 'alum_lag_{}'.format(i)] = hist_df.loc[year - i, 'alum']
    ...
    # target (alum production in year i)
    ...
    save_data = './data/ml_alum/{}/'.format(economy)
    ...
    for key, array in X_arrays.items():
        for fold, (train_index, test_index) in enumerate(tscv.split(array)):
            ...
            # # Charts
            # chart_df = hist_df[['alum']].copy() 
            # y_pred_lm_df = pd.DataFrame(y_pred_lm, index = hist_df.index[test_index], columns = ['alum_prediction_lm'])
            # y_pred_rr_df = pd.DataFrame(y_pred_rr, index = hist_df.index[test_index], columns = ['alum_prediction_rr'])
            # y_pred_lasso_df = pd.DataFrame(y_pred_lasso, index = hist_df.index[test_index], columns = ['alum_prediction_lasso'])
    ...
    for q in range(model_sort.shape[0]):
        ...
        if pd.Series(features).str.contains('alum_lag').any():
    ...
    saved_predictions = hist_df.copy().reset_index()[['year', 'alum']]
    saved_predictions['model'] = 'Historic aluminium production'
    ...
    for i in range(TOP_MODELS):
        ...
        if pd.Series(features).str.contains('alum_lag').any():
            ...
            for j in range(hist_df.index.max() + 1, temp_hist_df.index.max(), 1):
                # Create new line of X's to then predict alum
                new_X = temp_hist_df.loc[j, features]
                # Populate alum lagged cells
                for z in range(1, LAGS + 1, 1):
                    new_X['alum_lag_{}'.format(z)] = temp_hist_df.loc[j - z, 'alum']
                    temp_hist_df.loc[j, 'alum_lag_{}'.format(z)] = temp_hist_df.loc[j - z, 'alum']
                ...
                temp_hist_df.loc[j, 'alum'] = y_pred

            predicted = temp_hist_df.reset_index()[['year', 'alum']]
    ...
    # Revert alum to original unit (reverse log transformation)
    saved_predictions['alum'] = np.exp(saved_predictions['alum']) 
