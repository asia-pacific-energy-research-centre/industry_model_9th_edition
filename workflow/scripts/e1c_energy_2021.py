...
for economy in economy_list:
    energy_df = EGEDA_ind_total[(EGEDA_ind_total['economy'] == economy) &
                                ...
                                (EGEDA_ind_total['year'].isin([2019, 2020, 2021]))].copy().reset_index(drop = True)
    
    energy_df['cumulative'] = energy_df.groupby(['year', 'sector'])['energy'].cumsum()

    save_location = './data/EGEDA/energy_charts_2021/{}/'.format(economy)
    ...
    max_y = max(energy_df[energy_df['year'] == 2019]['energy'].sum(), 
                ...
                energy_df[energy_df['year'] == 2021]['energy'].sum())

    ax.set(title = economy + ' industry energy use 2021',
           xlabel = 'Year',
           ylabel = 'Petajoules',
           ylim = (0, max_y * 1.1))
    ...
    plt.tight_layout()
    plt.show()
    fig.savefig(save_location + economy + '_industry_energy_2021.png')
...
for economy in economy_list:
    ...
    for sector in EGEDA_ind_df['sector'].unique():
        energy_df = EGEDA_ind_df[(EGEDA_ind_df['economy'] == economy) &
                                 ...
                                 (EGEDA_ind_df['year'].isin([2019, 2020, 2021]))].copy().reset_index(drop = True)
